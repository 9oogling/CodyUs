  ...
  $.ajax({
    ...
    success: function (response) {
      if (response && response.data) {
        const { title, imageUrls, nickname, content, hashtags, price } = response.data;

        console.log('서버 응답 데이터:', response.data);

        // 게시물 정보 업데이트
        $('#post-detail').html(`<h1>${title}</h1><div class="post-username">작성자 : ${nickname}</div>`);

        const imageUrl = imageUrls.length > 0 ? imageUrls[0] : 'default-image-url.jpg';
        $('#post-image').html(`<div class="image-container"><img src="${imageUrl}" alt="${title}" class="img-fluid" /></div>`);
        ...
        const productImageUrl = imageUrls.length > 1 ? imageUrls[1] : 'default-image-url.jpg';
        ...
        $.ajax({
          ...
          success: function (statusResponse) {
            ...
            $('#like-button').html(
                isLiked ? '<img src="/images/redHeart.png" alt="red heart">️' : '<img src="/images/whiteHeart.png" alt="white heart">'
            );
  ...
  $('#like-button').click(function () {
    ...
    $.ajax({
      ...
      success: function () {
        ...
        let likes = parseInt(currentLikesText, 10) || 0;
        $('#post-likes').text(`${likes + (isLiked ? -1 : 1)}`);
        $('#like-button').html(
            isLiked ? '<img src="/images/whiteHeart.png" alt="white heart">' : '<img src="/images/redHeart.png" alt="red heart">'
        );
        $('#like-button').toggleClass('liked', !isLiked);
      },
      error: function (jqXHR, textStatus, errorThrown) {
